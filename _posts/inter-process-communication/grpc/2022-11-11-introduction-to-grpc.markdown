...
Let's talk about what protocol buffers (also called protobuf) are and how to use them. Protocol buffers is an open-source protocol developed by Google to *serialize* structured data in a way that works in different languages and on different platforms.
